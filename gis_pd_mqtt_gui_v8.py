...
class DatabaseViewDialog(QDialog):
    ...
    def __init__(self, db_manager, parent=None):
        ...
        # 添加生成PRPD图按钮 (只对周期数据有效)
        self.generate_prpd_button = QPushButton("生成PRPD图")
        self.generate_prpd_button.clicked.connect(self.generate_prpd_from_history)
    ...
    def generate_prpd_from_history(self):
        """从历史数据生成PRPD图"""
        # 确保数据类型是周期数据
        ...
        # 创建新的对话框显示PRPD图
        dialog = HistoricalPRPDDialog(self.query_results, self)
        dialog.exec()
...
class HistoricalPRPDDialog(QDialog):
    """历史PRPD图显示对话框"""
    def __init__(self, data, parent=None):
        super().__init__(parent)
        self.setWindowTitle("历史PRPD图")
        ...
        # 添加数据范围选择
        settings_layout.addWidget(QLabel("数据范围:"))
        ...
        settings_layout.addWidget(self.range_spin)
        settings_layout.addWidget(QLabel(f"/ {len(self.data)} 周期"))
        ...
        settings_layout.addWidget(self.show_sine_checkbox)
        
        # 添加正弦波振幅设置
        settings_layout.addWidget(QLabel("正弦波振幅:"))
        ...
        settings_layout.addWidget(self.sine_amplitude_spin)
    ...
    def update_chart(self):
        ...
        # 绘制图表
        if not all_data:
            self.axes.text(0.5, 0.5, "没有数据可显示", ha='center', va='center')
        else:
            # 合并所有周期的数据用于绘图
            flattened_data = []
            x_data = []
            ...
            if chart_type == "散点图":
                self.axes.scatter(x_data, flattened_data, alpha=0.7, s=10)
            elif chart_type == "线图":
                # 对于线图，按周期分别绘制
                for i, cycle_data in enumerate(all_data):
                    cycle_phases = np.linspace(0, 360, len(cycle_data))
                    # 仅当周期数不多时显示图例
                    if len(all_data) <= 10:
                        self.axes.plot(cycle_phases, cycle_data, linewidth=1.0, 
            ...
            if show_sine_wave:
                ...
                # 绘制正弦波
                self.axes.plot(x_sine, y_sine, 'r-', linewidth=1.5, alpha=0.7, label="参考正弦波")
            
            # 设置图表标题和轴标签
            self.axes.set_title(f"历史PRPD图 ({len(all_data)}个周期)")
            self.axes.set_xlabel("相位 (0~360°)")
            self.axes.set_ylabel("幅值 (V)")
