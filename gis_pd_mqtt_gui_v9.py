...
class HistoricalChartsDialog(QDialog):
    ...
    def __init__(self, data, parent=None):
        ...
        settings_layout.addWidget(self.show_sine_checkbox, 1, 0, 1, 2)
        
        # 添加正弦波振幅设置
        settings_layout.addWidget(QLabel("正弦波振幅:"), 1, 2)
        ...
        settings_layout.addWidget(self.sine_amplitude_spin, 1, 3)
    ...
    def draw_prpd(self, all_data, cycle_labels, chart_type, show_sine_wave, sine_amplitude):
        ...
        if chart_type == "PRPD散点图":
            self.axes_2d.scatter(x_data, flattened_data, alpha=0.7, s=10)
    ...
    def draw_prps(self, all_data, cycle_labels, color_scheme):
        ...
        if num_cycles == 0:
            self.axes_3d.text(0.5, 0.5, 0.5, "没有数据可显示", ha='center', va='center')
            return
            
        # 找到所有周期中最大的数据点数
        max_points = max(len(cycle_data) for cycle_data in all_data)
...
class MainWindow(QMainWindow):
    ...
    def clear_data(self):
        ...
        if self.canvas.axes_3d:
            ...
            self.canvas.axes_3d.set_zlabel("幅值 (mV)")
    ...
    def draw_prpd(self, accumulated_data):
        ...
        self.canvas.axes_2d.set_ylabel("幅值 (mV)")
